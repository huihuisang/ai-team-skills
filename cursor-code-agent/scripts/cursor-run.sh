...
# cursor-run.sh - Cursor Agent CLI wrapper script (code implementation role)
...
Options:
  ...
  -t, --timeout <seconds>    Timeout in seconds (default: 900)
  -f, --file <file>          Read prompt from file (recommended to avoid escaping issues)
  -r, --review               Use plan mode (read-only code analysis, no modifications)
...
Examples:
  ...
  cursor-run.sh -f /tmp/prompt.txt -o /tmp/result.txt -d ./project
  cursor-run.sh -r -f /tmp/review-prompt.txt -o /tmp/review.txt -d ./project
  cursor-run.sh --model gpt-5.3-codex-high -f /tmp/prompt.txt -d ./project
USAGE
    exit 0
}
...
# Review/plan mode: read-only code analysis without file modifications
...
echo "=== Cursor Code Agent Starting ===" >&2
...
# Execute cursor agent CLI (with timeout if available), optionally writing to output file
if [[ -n "$TIMEOUT_CMD" ]]; then
    if [[ -n "$OUTPUT_FILE" ]]; then
        $TIMEOUT_CMD "$TIMEOUT" agent "${CURSOR_ARGS[@]}" "$PROMPT" | tee "$OUTPUT_FILE"
    else
    ...
    if [[ -n "$OUTPUT_FILE" ]]; then
        agent "${CURSOR_ARGS[@]}" "$PROMPT" | tee "$OUTPUT_FILE"
    else
